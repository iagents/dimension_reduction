...
{
  LDA::LDA(Mat Data, Mat labels, int no_classes) {
    this->Data = Data;
    this->num_classes = no_classes;
    this->init(Data, labels, this->num_classes);
  }

  void LDA::init(Mat Data, Mat labels, int no_classes)
  {
  ...
  cv::Mat LDA::getWeights() {
    this->red_vals = Mat(this->vals, Range::all(), Range(0, this->num_classes - 1));
    this->weights = Mat(this->vecs, Range::all(), Range(0, this->num_classes - 1));
  ...
  cv::Mat LDA::project(Mat Data, Mat weights) {
    Mat newData;
